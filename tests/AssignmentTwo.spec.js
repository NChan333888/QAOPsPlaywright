...
// chatgpt
async function openFirstAvailableEvent(page) {
  const availableBookButton = page
    .getByTestId("book-now-btn")
    .filter({ hasText: "Book Now" })
    .first();

  await expect(availableBookButton).toBeVisible();
  await expect(availableBookButton).toBeEnabled();
  await availableBookButton.click();
}
...
test("refund eligible for single ticket booking", async ({ page }) => {
  ...
  // chatgpt - Select the first event that is still available for booking
  await openFirstAvailableEvent(page);
...
test("refund not eligible for group ticket booking", async ({ page }) => {
  ...
  // await page.locator('button:has-text("+")').click();
  // from chatgpt
  const increaseQuantityButton = page.locator('button:has-text("+")').first();
